...
export function BookingPage() {
  ...
  const confirmBooking = async (carId: number) => {
    ...
    try {
      ...
      setError(message);
      // Найдено вручную (2026-09-13): available-slots не знает, какой
      // автомобиль выберут дальше по шагам мастера, и не может заранее
      // убрать слоты, занятые ИМЕННО этим авто — конфликт вскрывается
      // только здесь. Раньше в этом случае мы всегда возвращали на выбор
      // времени и просили выбрать слот заново — тот же самый слот снова
      // проходил бы все проверки и снова падал на этой же машине, так что
      // пользователя закручивало в бесконечный цикл "дата/время ⇄ авто".
      // Слот тут ни при чём — обходим цикл, возвращая к выбору именно
      // машины, а слот и дату не трогаем.
      if (message.includes("Автомобиль уже записан")) {
        setStep("car");
        return;
      }
      // Иначе слот перестал быть свободным по другой причине (заняли все
      // посты / гонка за тем же временем) — тут действительно нужно заново
      // выбрать время, и повторный запрос available-slots уже не предложит
      // этот слот.
