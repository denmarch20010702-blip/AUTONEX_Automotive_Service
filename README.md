...
Проверено (2026-09-18): все три контейнера (`db`, `backend`, `frontend`) собираются и стартуют, `db` проходит healthcheck, `backend` отвечает на `/health`, `frontend` отдаёт страницу и достучивается до backend через CORS.
...
## Статус (обновлено 2026-09-18)

**Работает и проверено:** весь грейд A (сквозной happy path — запись клиента, конкурентно-безопасное бронирование слотов, статусная машина заявки, realtime-обновления между несколькими открытыми клиентами), весь обязательный грейд B (хранение шин, согласование доп. работ, напоминания, перенос/отмена записи, проактивные предложения ТО и сезонное напоминание про шины, экран станции, тесты-доказательства edge-cases), и собственная фича проекта целиком — роботизация станции + ИИ-диагностика + умная парковка (C1–C7): автотаймер с очередью задач на посту, живая трансляция прогресса, ИИ-диагностика доп. работ (rule-based по умолчанию, реальный LLM опционально), сквозной сценарий-доказательство полного цикла (`backend/scripts/prove_full_cycle.py`), 6 парковочных мест с автоприёмом/автопарковкой и наценкой за простой. 150 backend-тестов, `tsc -b --force` — 0 ошибок, GitHub Actions (`.github/workflows/ci.yml`) гоняет то же самое на каждый push/PR.

**Не сделано, сознательно, по времени перед сдачей:** доп. AUTONEX-фичи C8–C13 (Real-time Control Center, AI Service Scheduler, автономный режим, автосчета, кастомер-дэшборд, рейтинги) — каждая зависит от C7 и могла бы наращиваться дальше, но не входила в обязательный объём Задания 12. Полировка D2–D4 (фронтенд-тесты, логирование/метрики, доп. фильтры в истории) — открыта; D7 (гигиена зависимостей — lock-файлы + линтеры) закрыта полностью 2026-09-18. Защита ветки `main` на GitHub (обязательный CI перед мержем) — настроена частично, см. ниже. Подробности и обоснование порядка отсечения — в `logs/EXECUTION_PLAN.md` (раздел "Как режем, если время не хватит") и в разделе ниже.
...
8. **D2–D4** — фронтенд-тесты (сейчас 0, только backend), логирование/метрики, фильтры в истории визитов.
9. **`npm audit`** (2026-09-18, при добавлении `package-lock.json`) сообщает 4 уязвимости (3 moderate, 1 high) в `esbuild`/`react-router` — обе чинятся только через мажорное обновление (`vite@8`, `react-router-dom@7`) с breaking changes. Не тронуто прямо сейчас: перед сдачей нет времени на регрессионную проверку фронтенда после мажорного обновления роутера без единого фронтенд-теста (см. D2 выше) — риск сломать рабочий happy path выше пользы от исправления. Сознательно отложено, не забыто.
10. **Защита ветки `main` на GitHub** (обязательные CI-проверки `backend`/`frontend` перед мержем) — попытался настроить через `gh api`, но действие заблокировал классификатор прав auto-mode как изменение настроек репозитория. Нужно либо явное разрешение на этот вызов в следующей сессии, либо включить вручную: Settings → Branches → Add rule для `main` → Require status checks to pass → отметить `backend` и `frontend`.
11. **`eslint-plugin-react-refresh`** honestly нашёл 7 файлов, которые экспортируют одновременно и компонент, и хук/константу (`api/events.tsx`, `session/ClientSessionContext.tsx`, `session/NotificationContext.tsx`, `components/icons.tsx`, `components/booking/SlotPicker.tsx`) — это ломает точечный Vite Fast Refresh (полный перемонтаж вместо HMR) и структурно похоже на гипотезу из расследования более раннего "белого экрана" в этой же сессии (не подтверждённую напрямую, но правдоподобную). Оставлено предупреждением, не ошибкой CI — разнесение по файлам это отдельная рефакторинг-задача.
